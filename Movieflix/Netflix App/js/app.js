...
app.
controller('videosDisplayCtrl', function ($scope, $http) {

  $scope.showfulldata = false;
  $scope.moviefullDetails = function (fullDetails) {
    $scope.showfulldata = true;
    $scope.movieFullData = fullDetails;
    console.log($scope.movieFullData.title);
  }
  ...
  // Remove Items
  $scope.removeImdb = function (imdb) {
    for (var i = 0; i < $scope.movielists.length; i++) {
      if ($scope.movielists[i].imdbId == imdb)
        $scope.movielists.splice(i, 1);
    }
  }

  // Update Items

  $scope.updateImdb = function (imdb) {
    for (var i = 0; i < $scope.movielists.length; i++) {
      if ($scope.movielists[i].imdbId == imdb) {
        $scope.taskname = $scope.movielists[i].title;
        $scope.genre = $scope.movielists[i].genre;
        $scope.taskyear = $scope.movielists[i].year;

      }
      //$scope.movielists.splice(i);
    }
  }
...
app.
controller('addtitleDisplayCtrl', function ($scope, $http) {
  $scope.details = [];
  $scope.counter = 0;
  $scope.addTitle = function (taskname, description, taskyear, ) {
  ...
  $scope.removeTitle = function (id) {
    for (var i = 0; i < $scope.details.length; i++) {
      if ($scope.details[i].id == id)
        $scope.details.splice(i);
    }
  }

  $scope.updateTitle = function (index) {
    for (var i = 0; i < $scope.details.length; i++) {
      if ($scope.details[i].id == index) {
        $scope.taskname = $scope.details[i].title;
        $scope.description = $scope.details[i].description;
        $scope.taskyear = $scope.details[i].taskyear;
      }
    }
  }
});
...
controller('loginCtrl', function ($scope, $http, $location) {
  $scope.checkuser = function (username, password) {
    ...
    if ($scope.username == 'admin' && $scope.username == 'admin') {
      $scope.redirectAdmin();
      //$location.path('/addtitle'); 
    }
    if ($scope.username == 'user' && $scope.username == 'user') {
      $scope.redirect();
...
$(function () {
  ...
  jQuery('.slider1').lbSlider({
    leftBtn: '.sa-left1', // left button selector
    rightBtn: '.sa-right1', // right button selector
    visible: 5, // visible elements quantity
    autoPlay: true, // autoscroll
    autoPlayDelay: 10 // delay of autoscroll in seconds
  });

  jQuery('.sliderthree').lbSlider({
    leftBtn: '.sa-left3', // left button selector
    rightBtn: '.sa-right3', // right button selector
    visible: 5, // visible elements quantity
    autoPlay: true, // autoscroll
    autoPlayDelay: 10 // delay of autoscroll in seconds
  });

  jQuery('.sliderfour').lbSlider({
    leftBtn: '.sa-left4', // left button selector
    rightBtn: '.sa-right4', // right button selector
    visible: 5, // visible elements quantity
    autoPlay: true, // autoscroll
    autoPlayDelay: 10 // delay of autoscroll in seconds
  });
